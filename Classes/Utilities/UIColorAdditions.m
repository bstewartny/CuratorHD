...
+ (NSArray*) cssColorNames
{
	...
	NSString *aa = [NSString stringWithCString: colorNameDB encoding: NSASCIIStringEncoding];
	
	NSArray * a=[aa componentsSeparatedByString:@","];
	
	for(NSString * c in a)
	{
		NSArray * cc=[c componentsSeparatedByString:@"#"];
		if([cc count] ==2)
		{
			[tmp addObject:[cc objectAtIndex:0]];
		}
	}
