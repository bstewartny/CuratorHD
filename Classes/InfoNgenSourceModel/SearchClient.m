...
- (NSMutableArray*) getSavedSearchesForUser
{
	@try 
	{
		return [self getSavedSearchesForUserImpl];
	}
	@catch (NSException * e) {
...
- (NSMutableArray*) getSavedSearchesForUserImpl
{

	NSMutableArray * searches=[[NSMutableArray alloc] init];
	
	LoginTicket * ticket=[[LoginTicket alloc] initWithUsername:self.username password:self.password useCachedCookie:YES];
	...
	if(data)
	{
		...
		for (CXMLElement *searchItem in searchItems) {
			...
			RSSSavedSearch * savedSearch=[[RSSSavedSearch alloc] initWithName:Title withID:ID withUrl:[NSString stringWithFormat:@"http://rss.infongen.com/search.rss?name=%@",escapedTitle]];
			//savedSearch.image=[UIImage imageNamed:@"icon.png"];
