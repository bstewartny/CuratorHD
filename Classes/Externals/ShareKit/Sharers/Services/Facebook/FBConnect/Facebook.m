       ...
       delegate:(id<FBRequestDelegate>)delegate 
{
	...
	NSData * data=[request sendSynchronousRequest:&error];
	
	if(data==nil)
	{
		NSLog(@"got nil data from sendSynchronousRequest");
	}
	
	if(error)
	{
		NSLog(@"Error for url: %@",url);
	...
	if(data!=nil)
	{
		...
		id json=[request parseJsonResponse:data error:&error];
...
- (NSData*)getDataWithMethodName:(NSString *)methodName
                    andParams:(NSMutableDictionary *)params
                andHttpMethod:(NSString *)httpMethod
                  andDelegate:(id <FBRequestDelegate>)delegate {
	NSString * fullURL = [kRestApiURL stringByAppendingString:methodName];
	return [self getUrlData:fullURL params:params httpMethod:httpMethod delegate:delegate];
}
...
- (id)getJsonWithGraphPath:(NSString *)graphPath
					  
                 andDelegate:(id <FBRequestDelegate>)delegate {
	
	return [self getJsonWithGraphPath:graphPath
					 andParams:[NSMutableDictionary dictionary]
				 andHttpMethod:@"GET"
				   andDelegate:delegate];
}
...
- (id)getJsonWithGraphPath:(NSString *)graphPath
	...
	NSLog(@"getJsonWithGraphPath: %@",fullURL);
	
	return [self getJson:fullURL params:params httpMethod:httpMethod delegate:delegate];
}
